<script lang="ts">
  import { t, toggleLocale, getLocale } from '$lib/i18n/index.svelte';
  import { Cookie, Globe } from '@lucide/svelte';

  const OPEN_CONSENT_PREFERENCES_EVENT = 'ethoz:open-consent-preferences';

  function openConsentPreferences() {
    window.dispatchEvent(new CustomEvent(OPEN_CONSENT_PREFERENCES_EVENT));
  }

  const social = [
    {
      name: 'LinkedIn',
      href: 'https://www.linkedin.com/company/ethozcl/',
      svg: `<svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="size-4"><title>LinkedIn</title><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>`
    },
    {
      name: 'Facebook',
      href: 'https://web.facebook.com/profile.php?id=1083964671464526',
      svg: `<svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="size-4"><title>Facebook</title><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>`
    },
    {
      name: 'Instagram',
      href: 'https://www.instagram.com/ethoz.cl/',
      svg: `<svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="size-4"><title>Instagram</title><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z"/></svg>`
    },
    {
      name: 'Google',
      href: 'https://share.google/kmwkmo8To6711zCgB',
      svg: `<svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="size-4"><title>Google</title><path d="M12.48 10.92v3.28h7.84c-.24 1.84-.853 3.187-1.787 4.133-1.147 1.147-2.933 2.4-6.053 2.4-4.827 0-8.6-3.893-8.6-8.72s3.773-8.72 8.6-8.72c2.6 0 4.507 1.027 5.907 2.347l2.307-2.307C18.747 1.44 16.133 0 12.48 0 5.867 0 .307 5.387.307 12s5.56 12 12.173 12c3.573 0 6.267-1.173 8.373-3.36 2.16-2.16 2.84-5.213 2.84-7.667 0-.76-.053-1.467-.173-2.053H12.48z"/></svg>`
    },
    {
      name: 'YouTube',
      href: 'https://www.youtube.com/channel/UCYeWEdqonYWKvja78_HM2TA',
      svg: `<svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="size-4"><title>YouTube</title><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>`
    },
  ];
</script>

<footer class="border-t border-foreground/10 bg-background">
  <!-- Upper footer -->
  <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-12">
    <div class="grid grid-cols-2 gap-x-6 gap-y-7 sm:grid-cols-3 lg:grid-cols-6">

      <!-- Brand + social -->
      <div class="col-span-2 sm:col-span-3 lg:col-span-2">
        <a href="/" class="inline-flex items-center gap-2">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" class="size-6" aria-hidden="true">
            <rect x="2" y="7" width="18" height="22" rx="5" fill="none" stroke="currentColor" stroke-width="2.2" class="text-foreground"/>
            <rect x="12" y="3" width="18" height="22" rx="5" fill="none" stroke="currentColor" stroke-width="2.2" class="text-primary"/>
          </svg>
          <span class="font-heading text-lg font-bold tracking-tight"><span class="text-foreground">Etho</span><span class="text-primary">z</span></span>
        </a>
        <p class="mt-3 max-w-xs text-sm leading-relaxed text-muted-foreground">
          {t('footer.tagline')}
        </p>
        <div class="mt-4 flex items-center gap-3">
          {#each social as s (s.name)}
            <a
              href={s.href}
              target="_blank"
              rel="noopener noreferrer"
              aria-label={s.name}
              class="flex size-9 items-center justify-center rounded-full bg-foreground/5 text-muted-foreground transition-colors hover:bg-foreground/10 hover:text-foreground"
            >
              {@html s.svg}
            </a>
          {/each}
        </div>
      </div>

      <!-- Producto -->
      <div>
        <h3 class="text-xs font-semibold uppercase tracking-wider text-foreground">{t('footer.product')}</h3>
        <ul class="mt-3 flex flex-col gap-2">
          <li><a href="/productos" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.features')}</a></li>
          <li><a href="/integraciones" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.integrations')}</a></li>
          <li><a href="/comparativa" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.comparison')}</a></li>
          <li><a href="/calculadora-roi" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.roi')}</a></li>
          <li><a href="/proyecciones" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.projections')}</a></li>
          <li><a href="/como-contratar" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.pricing')}</a></li>
        </ul>
      </div>

      <!-- Recursos -->
      <div>
        <h3 class="text-xs font-semibold uppercase tracking-wider text-foreground">{t('footer.resources_section')}</h3>
        <ul class="mt-3 flex flex-col gap-2">
          <li><a href="/ley-21719" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.law')}</a></li>
          <li><a href="/circular-30" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.circular')}</a></li>
          <li><a href="/cumplimiento" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.compliance')}</a></li>
          <li><a href="/glosario" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.glossary')}</a></li>
          <li><a href="/recursos" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.downloads')}</a></li>
          <li><a href="/seguridad-datos" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.security')}</a></li>
        </ul>
      </div>

      <!-- Empresa -->
      <div>
        <h3 class="text-xs font-semibold uppercase tracking-wider text-foreground">{t('footer.company')}</h3>
        <ul class="mt-3 flex flex-col gap-2">
          <li><a href="/nosotros" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.about')}</a></li>
          <li><a href="/blog" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.blog')}</a></li>
          <li><a href="/contacto" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.contact')}</a></li>
          <li><a href="/demo" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('nav.cta')}</a></li>
        </ul>
      </div>

      <!-- Para ti -->
      <div>
        <h3 class="text-xs font-semibold uppercase tracking-wider text-foreground">{t('footer.for_you')}</h3>
        <ul class="mt-3 flex flex-col gap-2">
          <li><a href="/para-sostenedores" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.operators')}</a></li>
          <li><a href="/para-directores" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.directors')}</a></li>
          <li><a href="/para-porteros" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.porters')}</a></li>
          <li class="mt-2 border-t border-foreground/10 pt-3"><a href="/privacidad" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.privacy')}</a></li>
          <li><a href="/terminos" class="text-sm text-muted-foreground transition-colors hover:text-foreground">{t('footer.terms')}</a></li>
        </ul>
      </div>

    </div>
  </div>

  <!-- Bottom bar -->
  <div class="border-t border-foreground/10">
    <div class="mx-auto flex max-w-7xl flex-col items-center justify-between gap-2 px-4 py-3 sm:flex-row sm:px-6 lg:px-8">
      <p class="text-xs text-muted-foreground">{t('footer.rights')} · {t('footer.tagline')}</p>
      <div class="flex items-center gap-1">
        <button
          type="button"
          aria-haspopup="dialog"
          onclick={openConsentPreferences}
          class="inline-flex min-h-11 items-center gap-1.5 rounded-md px-3 py-2 text-xs text-muted-foreground transition-colors hover:bg-foreground/5 hover:text-foreground"
        >
          <Cookie class="size-3.5" aria-hidden="true" />
          {t('footer.cookies')}
        </button>
        <button
          type="button"
          onclick={toggleLocale}
          class="inline-flex min-h-11 items-center gap-1.5 rounded-md px-3 py-2 text-xs text-muted-foreground transition-colors hover:bg-foreground/5 hover:text-foreground"
        >
          <Globe class="size-3" aria-hidden="true" />
          {getLocale() === 'es' ? t('lang.en') : t('lang.es')}
        </button>
      </div>
    </div>
  </div>
</footer>
